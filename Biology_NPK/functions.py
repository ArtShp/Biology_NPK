...
def sequence_global_alignment(s1: str, s2: str):
    ...
    for i in range(1, len(s1)+1):
        table[i][0] = table[i-1][0] + Score(s1[i-1], '*')
    for j in range(1, len(s2)+1):
        table[0][j] = table[0][j-1] + Score('*', s2[j-1])

    for i in range(1, len(s1)+1):
        for j in range(1, len(s2)+1):
            table[i][j] = max(table[i-1][j] + Score(s1[i-1], '*'),
                              table[i][j-1] + Score('*', s2[j-1]),
                              table[i-1][j-1] + Score(s1[i-1], s2[j-1]))

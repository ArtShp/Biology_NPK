...
def count_cell_score(symbol1: str, symbol2: str, mode: int = 0):
    """Функция для подсчёта стоимости текущей ячейки."""
    """ВАЖНО! Передаются только символы, а не строки"""
    # ----- Сделать список в конфиге возможных таблиц выравнивания(режимов)(Возможно) + режими - не номера, а слова[Union]
    # ----- Либо гарантировать, что сюда передаются только символы(Сомнительно)
    if len(symbol1) != 1 or len(symbol2) != 1 or mode not in [0, 1]:
        # ----- Делать что-то другое. Возможно добавить свой обработчик ошибок!
        return None

    if mode == 0:
        """Используем базовую схему"""
        if '*' in [symbol1, symbol2]:
            return -1
        ...
        """Используем BLOSUM62"""
        # ----- Хранить в коде не как словарь, а выгружать из файла(Возможно)
        ...
        # ----- Обработчик ошибок не здесь(Не знаю, может и тут лучше, или в основном цикле)
        try:
            return int(align[symbol1][symbol2])
        except KeyError:
            # ----- Делать что-то другое. Возможно добавить свой обработчик ошибок!
            return None
    else:
...
def sequence_global_alignment(s1: str, s2: str, mode: int = 0):

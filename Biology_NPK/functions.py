...
def sequence_global_alignment(s1, s2):
    """Функция глобального выравнивания двух генетических цепочек
       по алгоритму Нидлмана-Вунша"""
    # D(i, j) = max(D(i-1, j) + Score(S[i], -), D(i, j-1) + Score(-, S[j]), D(i-1, j-1) + Score(S[i], S[j]))
    ...
    for i in range(1, len(s1)+1):
        for j in range(1, len(s2)+1):
            table[i][j] = max(table[i-1][0] + Score(s1[i-1], '*'),
                              table[0][j-1] + Score('*', s2[j-1]),
